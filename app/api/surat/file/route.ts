...
export async function GET(request: Request) {
  ...
  if (!normalized.startsWith("uploads/surat/") || normalized.includes("..")) {
  ...
  try {
    ...
    return NextResponse.json({ message: "File tidak ditemukan. Jika aplikasi baru diredeploy, pastikan folder uploads memakai persistent storage." }, { status: 404 });
  }
}
